...
function normalizeTrack(x: unknown): Track {
  ...
  const createdAt = str(obj.createdAt) ?? str(obj.date) ?? undefined;
...
export default function Index({ lang, setLang }: { lang: Lang; setLang: (l: Lang) => void }) {
  ...
  // reshape order when list or channel changes
  useEffect(() => {
    setOrder(shuffledIndices(currentList.length));
    setCurrentIndex(0);
  }, [channel, currentList.length]);

  const currentTrack = useMemo(() => {
    if (!currentList.length || !order.length) return null;
    const idx = order[currentIndex % order.length] ?? 0;
    return currentList[idx] || null;
  }, [currentList, order, currentIndex]);
  ...
  const startRadio = () => {
    ...
    // pick a random starting position in the shuffled order
    const pos = Math.floor(Math.random() * Math.max(1, order.length));
    setCurrentIndex(pos);
  ...
  const playNext = () => {
    if (currentList.length === 0 || order.length === 0) return;
    const nextPos = (currentIndex + 1) % order.length;
    setCurrentIndex(nextPos);
    const idx = order[nextPos] ?? 0;
    const next = currentList[idx];
    if (next?.url) playUrl(next.url);
  };
  ...
  const playPrev = () => {
    if (currentList.length === 0 || order.length === 0) return;
    const prevPos = (currentIndex - 1 + order.length) % order.length;
    setCurrentIndex(prevPos);
    const idx = order[prevPos] ?? 0;
  ...
  // Top 10 newest in selected category (heuristic: sort by createdAt if available, else by filename descending)
  const top10 = useMemo(() => {
    const list = currentList.map((t) => ({ t, key: t.createdAt || (t.url || '').split('/').pop() || t.title }));
    list.sort((a, b) => String(b.key).localeCompare(String(a.key)));
    return list.slice(0, 10).map((x) => x.t);
  }, [currentList]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-neutral-900 to-black">
      <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-black/60 border-b border-neutral-800">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link to="/artykuly" className="text-sm text-neutral-300 hover:text-white underline" aria-label="Przejdź do artykułów">
              Artykuły
            </Link>
                ...
                if (streamUrl) {
                  ...
                  if (streamUrl) {
                    ...
                    if (el) {
                      ...
                      el.play().then(() => setIsRadioPlaying(true)).catch(() => {});
                    }
                  }
                } else {
                  startRadio();
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-10">
        ...
        <section className="rounded-2xl border border-neutral-800 p-6">
          <Tabs defaultValue="analizy">
            <TabsList className="bg-neutral-900 border border-neutral-800">
              <TabsTrigger value="analizy">Analizy</TabsTrigger>
              <TabsTrigger value="fakty">Fakty</TabsTrigger>
              <TabsTrigger value="opowiesci">Opowieści</TabsTrigger>
              <TabsTrigger value="edukacja">Edukacja prawna</TabsTrigger>
            </TabsList>

            <TabsContent value="analizy" className="mt-4">
              <Teaser title="Analiza psychologiczna" text={content?.data.analiza_psychologiczna?.snippet || ''} to="/analizy" readMore="Czytaj dalej" />
              <Separator className="my-4 bg-neutral-800" />
              <Teaser title="Raport analityczny" text={content?.data.raport_analityczny?.snippet || ''} to="/analizy" readMore="Czytaj dalej" />
            </TabsContent>

            <TabsContent value="fakty" className="mt-4">
              <Teaser title="5 faktów (TL;DR)" text={content?.data.tldr_5_faktow?.snippet || ''} to="/fakty" readMore="Czytaj dalej" />
              <Separator className="my-4 bg-neutral-800" />
              <Teaser title="Kluczowe fakty" text={content?.data.kluczowe_fakty?.snippet || ''} to="/fakty" readMore="Czytaj dalej" />
            </TabsContent>

            <TabsContent value="opowiesci" className="mt-4">
              <Teaser title="Polana kłamstw" text={content?.data.polana_klamstw?.snippet || ''} to="/opowiesci" readMore="Czytaj dalej" />
              <Separator className="my-4 bg-neutral-800" />
              <Teaser title="Źródła i kronika" text={content?.data.timeline_combined?.snippet || ''} to="/opowiesci" readMore="Czytaj dalej" />
            </TabsContent>

            <TabsContent value="edukacja" className="mt-4">
              <Teaser title="Służebność vs Dożywocie" text={content?.data.edukacja_prawna?.snippet || ''} to="/edukacja" readMore="Czytaj dalej" />
            </TabsContent>
          </Tabs>
        </section>

        <section id="radio" className="rounded-2xl border border-neutral-800 p-6 space-y-4">
          <h3 className="text-2xl font-bold">Radio</h3>
          <p className="text-neutral-400">Odtwarzanie losowe w obrębie wybranego kanału. Naciśnij Start.</p>

          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm text-neutral-400">Kanał:</span>
            {(['podcast', 'music', 'mix'] as Channel[]).map((c) => (
              ...
              >
                {c === 'podcast' ? 'Podcasty' : c === 'music' ? 'Muzyka' : 'Mix'}
              </Button>
            ))}
          </div>
          ...
          <Card className="bg-neutral-950 border-neutral-800">
            <CardHeader>
              <CardTitle>Teraz gra</CardTitle>
              <CardDescription className="text-neutral-400">
                {isExternalStream ? 'Zewnętrzny strumień' : channel === 'podcast' ? 'Podcasty' : channel === 'music' ? 'Muzyka' : 'Mix'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isExternalStream ? (
                  ...
                  <div>
                    ...
                    {currentList.length > 0 ? (
                      <div className="text-xs text-neutral-500 mt-1">
                        Pozycja: {(currentIndex % Math.max(1, order.length)) + 1} / {order.length || currentList.length}
                      </div>
                    ) : null}
                  </div>
                </div>
              ) : (
                <p className="text-sm text-neutral-500">Gotowe do odtwarzania. Naciśnij „Start”.</p>
              )}
          ...
          <div className="rounded-2xl border border-neutral-800 p-4">
            <h4 className="font-semibold mb-2">Top 10 najnowszych ({channel === 'podcast' ? 'Podcasty' : channel === 'music' ? 'Muzyka' : 'Mix'})</h4>
          ...
          <div className="grid md:grid-cols-[1fr,auto] gap-2">
            <div className="space-y-2">
              <Label htmlFor="stream">Zewnętrzny URL strumienia (opcjonalnie)</Label>
              <Input
                id="stream"
              ...
              <Button
                onClick={() => {
                  ...
                  if (tempStreamUrl) {
                    ...
                    const el = audioRef.current;
                    if (el) {
                      el.src = encodeURI(tempStreamUrl);
                      el.play().then(() => setIsRadioPlaying(true)).catch(() => {});
                    }
                  }
                }}
        ...
        <section className="rounded-2xl border border-neutral-800 p-6">
          <h3 className="text-2xl font-bold">Społeczność</h3>
          <p className="text-neutral-400 mb-3">Komentarze zapisywane lokalnie (bez logowania i realtime).</p>
  ...
  return (
    <Card className="bg-neutral-950 border-neutral-800">
      ...
      <CardContent>
        ...
        <div className="mt-3">
          <Link to={to} className="inline-flex items-center text-sm font-medium" style={{ color: '#f59e0b' }}>
            {readMore} →
          </Link>
        </div>
      </CardContent>
